...
def get_schedule_days():
    ...
    try:
        ...
        return jsonify([{
            ...
            'lessons_count': day.lessons_count
        } for day in days])
...
def search_groups():
    ...
    try:
        ...
        groups = db.session.query(Schedule.group_name) \
            .filter(Schedule.group_name.ilike(f'%{query}%')) \
            .distinct() \
...
def search_teachers():
    ...
    try:
        ...
        teachers = db.session.query(Schedule.teacher_name) \
            .filter(Schedule.teacher_name.ilike(f'%{query}%')) \
...
def search_rooms():
    ...
    try:
        ...
        rooms = db.session.query(Schedule.auditory) \
            .filter(Schedule.auditory.ilike(f'%{query}%')) \
            .filter(Schedule.auditory != '') \
            .distinct() \
            .order_by(Schedule.auditory) \
            .limit(10) \
            .all()
